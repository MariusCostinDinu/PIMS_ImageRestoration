...
def random_bbox(config, batch_size, fixed_position=False):
    ...
    """
    img_height, img_width, _ = config['image_shape']    # 256, 256
    h, w = config['mask_shape']                         # 128, 128
    margin_height, margin_width = config['margin']      # 0, 0
    maxt = img_height - margin_height - h               # 128
    maxl = img_width - margin_width - w                 # 128
    bbox_list = []
    if config['mask_batch_same']:                       # True
        if fixed_position:
            t, l = tuple(config['mask_pos'])
        else:
            t = np.random.randint(margin_height, maxt)      # rand(0, 128)
            l = np.random.randint(margin_width, maxl)       # rand(0, 128)
        bbox_list.append((t, l, h, w))                  # (rand1, rand2, 128, 128)
        # in acest moment bbox_list e o lista cu doar 1 element: elementul este acel (t,l,h,w)
        bbox_list = bbox_list * batch_size              # duplica elementul deja existent in bbox_list de batch_size ori
        # in acest moment bbox_list are batch_size elemente identice
    else:
...
def mask_image(x, bboxes, config, fixed_position=False):
    height, width, _ = config['image_shape']
    max_delta_h, max_delta_w = [0, 0] if fixed_position else config['max_delta_shape']
...
def add_padding_to_all_sides(batch, padding_size):
    padding_size = int(padding_size)
    padded_batch = torch.zeros(batch.size(0), batch.size(1), batch.size(2) + padding_size * 2, batch.size(3) + padding_size * 2)
    padded_batch[:, :, padding_size:(padded_batch.size(2)-padding_size), padding_size:(padded_batch.size(3)-padding_size)] += batch
    return padded_batch


# masca de dimensiuni [1,1,128,128] cu valori de 1 sau 0.9 pe margini si valori ce tind spre 0 in mijloc (vreo 0.01 cea mai mica, de la indecsii [0,0,128/2-1,128/2-1])
# In mod evident, imaginile nu vor fi mascate astfel in procesul inpainting-ului. Masca asta se foloseste doar pentru calcularea de loss function, unde se doreste ca 
# pixelii marginasi si conteze cel mai mult la loss, iar cei mijlocasi mult mai putin (tinzand spre 0 weight).
